...
credentials = ServiceAccountCredentials.from_json_keyfile_name('DSfinal-main/期末/flask/sincere-venture-419101-185a586d9858.json', scope)
...
# 讀取經緯度數據
file_path = 'DSfinal-main/期末/flask/經緯度參考.txt'
#辦公室
...
for row in data:
    prompt += f"某地溫度，濕度，空氣品質，冷氣狀況和除濕機狀況，地點{row['地點']}時間{row['時間']}，溫度{row['溫度(°C)']}，濕度{row['濕度']}，空氣品質{row['空氣品質']}，冷氣狀況{row['冷氣狀況(12燈)']}，除濕機狀況{row['除濕機狀況(16燈)']}；"
